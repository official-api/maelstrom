...
const SIM = (() => {
  ...
  function buildRocket() {
    ...
    for (let i = 0; i < 4; i++) {
      const orbitalAngle = (i / 4) * Math.PI * 2 + Math.PI / 4; // 45°, 135°, 225°, 315°
  ...
  function makeFixedFin(mat) {
    // Clipped delta fin.
    // The parent Group handles the orbital angle around the rocket body.
    // This mesh is built in the XZ plane (shape in X=span, Y=chord)
    // and positioned so its root edge aligns with the body radius (+X from group centre).
    // ExtrudeGeometry extrudes along Z (thickness), so we rotate the mesh -90° around X
    // to make the fin plane face radially outward (flat face visible from the side).
    const shape = new THREE.Shape();
    // Root at origin, span goes in +X, chord in +Y
    shape.moveTo(0,    0);      // root leading edge
    shape.lineTo(0,   -0.6);   // root trailing edge
    shape.lineTo(1.7, -0.9);   // clipped trailing tip
    shape.lineTo(1.8, -0.5);   // outer tip
    shape.lineTo(1.1,  0);     // leading edge tip
    shape.lineTo(0,    0);
    ...
    // Shift root to body surface radius
    fin.position.set(0.45, 0, -0.035);
    // Rotate so the fin stands radially outward (XZ plane → XY plane of the world)
  ...
  function makeCtrlFin(mat) {
    ...
    // Same coordinate convention as makeFixedFin — group orbital angle handles spin.
    const shape = new THREE.Shape();
    shape.moveTo(0,   0);
    shape.lineTo(0,  -0.7);
